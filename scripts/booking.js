...
function loadAvailableSlots(clearHint) {
    ...
    // Ejecutar cleanup de turnos expirados ANTES de cargar slots
    fetch(API_URL + "?action=ejecutarCleanup&token=" + encodeURIComponent(API_TOKEN))
        .then(function(r){return r.json()})
        .catch(function(e){ console.warn("Cleanup falló (no crítico):", e); })
        .then(function() {
            return fetch(API_URL + "?action=obtenerTurnos&duracionFilas=" + duracionFilas + "&token=" + encodeURIComponent(API_TOKEN));
        })
        .then(function(r){return r.json()})
        .then(function(data) {
           ...
           var slots = data.turnos || [];
            if (!slots.length) { 
                var waNoSlots = encodeURIComponent('Hola! Queria consultar por disponibilidad de turnos para ' + tratamiento);
                var noSlotsMsg = duracionFilas > 1 
                    ? "No hay horarios con espacio suficiente de <strong>" + duracionTexto + "</strong> seguidas en el mismo día. <a href='https://wa.me/" + WHATSAPP_NUMBER + "?text=" + waNoSlots + "' target='_blank' style='color:#FFD700;text-decoration:underline'>Escribinos por WhatsApp</a> para consultar."
                    : "No hay turnos disponibles. <a href='https://wa.me/" + WHATSAPP_NUMBER + "?text=" + encodeURIComponent('Hola! Queria consultar por disponibilidad de turnos.') + "' target='_blank' style='color:#FFD700;text-decoration:underline'>Escribinos por WhatsApp</a>.";
            ...
            if (!dateKeys.length) {
                slotsGrid.innerHTML = "<div class='no-slots'>No se encontraron turnos con fechas válidas. <a href='https://wa.me/" + WHATSAPP_NUMBER + "?text=" + encodeURIComponent('Hola! Queria consultar por disponibilidad de turnos.') + "' target='_blank' style='color:#FFD700;text-decoration:underline'>Escribinos por WhatsApp</a>.</div>";
                return;
            }
            ...
            if (!sortedDates.length) {
                slotsGrid.innerHTML = "<div class='no-slots'>No hay turnos disponibles dentro del rango de <strong>" + MAX_MESES_RESERVA + " mes" + (MAX_MESES_RESERVA > 1 ? "es" : "") + "</strong>. <a href='https://wa.me/" + WHATSAPP_NUMBER + "?text=" + encodeURIComponent('Hola! Queria consultar disponibilidad de turnos fuera del rango online.') + "' target='_blank' style='color:#FFD700;text-decoration:underline'>Escribinos por WhatsApp</a>.</div>";
                return;
            }
        ...
        .then(function(data) {
            ...
            if (data.status === "CONFIRMADO") {
               ...
               var msgError = encodeURIComponent('Hola! No me salió el turno. Queria reservar: ' + treatmentName + ' el ' + fecha + ' a las ' + horario + '. Por favor ayudenme.');
                ...
                if (senaDivErr) {
                    senaDivErr.style.display = "block";
                    senaDivErr.innerHTML = '<div style="background:rgba(0,0,0,0.15);border-radius:16px;padding:32px 24px;max-width:550px;margin:0 auto;text-align:center">'
                        + '<div style="font-size:3rem;margin-bottom:16px">⚠️</div>'
                        + '<h3 style="color:#FFD700;margin-bottom:12px">Error al reservar</h3>'
                        + '<p>Ocurrió un error inesperado. No te preocupes, contactanos por WhatsApp y te ayudamos a gestionar el turno.</p>'
                        ...
                        + '<a href="' + waLink + '" target="_blank" style="display:inline-block;background:#25D366;color:white;padding:14px 28px;font-size:1rem;border-radius:50px;text-decoration:none;margin-top:10px">📱 Contactar por WhatsApp</a>'
                    ...
                    showError("Ocurrió un error inesperado. Contactanos por WhatsApp.");
                }
            }
        })
        .catch(function() {
...
function showBookingSuccess(nombre, tratamiento, fecha, hora) {
    ...
    if(successDiv){
        ...
        var successHTML = '<div style="padding:28px 24px 40px 24px;max-width:550px;margin:0 auto;text-align:center">';
