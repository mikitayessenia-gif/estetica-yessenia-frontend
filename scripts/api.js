        ...
        .then(function(data) {
            ...
            if (data.error && data.error.includes("infringen las reglas de validaci")) {
                ...
                if (senaDiv) {
                    ...
                    var waMsgServer = encodeURIComponent('Hola! La agenda necesita una actualizacion rapida en Google Sheets. Por favor ayudenme.');
                    var waLinkServer = 'https://wa.me/' + WHATSAPP_NUMBER + '?text=' + waMsgServer;
                    senaDiv.innerHTML = '<div style="background:rgba(255,193,7,0.15);border:2px solid #ffc107;border-radius:16px;padding:24px;text-align:center"><div style="font-size:2rem;margin-bottom:12px">⚠️</div><h3 style="color:#ffc107;margin-bottom:8px">Error de configuracion del servidor</h3><p style="opacity:0.9;font-size:0.9rem;max-width:450px;margin:0 auto 16px">La agenda necesita una actualizacion rapida en Google Sheets.</p><a href="' + waLinkServer + '" target="_blank" style="display:inline-block;background:#25D366;color:white;padding:12px 24px;border-radius:50px;text-decoration:none;font-weight:600">📱 Contactar por WhatsApp</a></div>';
                }
                return;
            }
